...
const BookingConfirmationPage = () => {
  ...
  const { data: booking } = useQuery<Booking>(
    ...
    {
      enabled: !!id,
      retry: 1, // Only retry once to avoid multiple error flashes
      retryDelay: 300, // Short delay between retries
      refetchOnWindowFocus: false, // Don't refetch when window regains focus
      onError: (err: any) => {
        console.log('Error fetching booking:', err);
        // Don't set error state here, as we'll fall back to mock data
        setIsLoading(false);
      },
      onSuccess: () => {
        setIsLoading(false);
